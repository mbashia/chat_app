defmodule ChatSystem.Texts do
  @moduledoc """
  The Texts context.
  """

  import Ecto.Query, warn: false
  alias ChatSystem.Repo

  alias ChatSystem.Texts.Text

  def subscribe() do
    Phoenix.PubSub.subscribe(ChatSystem.PubSub, "mytopic")
  end

  #  the code below broadcasts a message to the the topic "mytopic"
  def publish({:ok, text}, tag) do
    Phoenix.PubSub.broadcast(ChatSystem.PubSub, "mytopic", {tag, text})

    {:ok, text}
  end

  def publish({:error, _changeset} = error, _tag), do: error

  def create_text(attrs \\ %{}) do
    %Text{}
    |> Text.changeset(attrs)
    |> Repo.insert()
    |> publish(:message_created)
  end

  @doc """
  Returns the list of texts.

  ## Examples

      iex> list_texts()
      [%Text{}, ...]

  """

  def list_texts do
    Repo.all(Text)
  end

  @doc """
  Gets a single text.

  Raises `Ecto.NoResultsError` if the Text does not exist.

  ## Examples

      iex> get_text!(123)
      %Text{}

      iex> get_text!(456)
      ** (Ecto.NoResultsError)

  """
  def get_text!(id), do: Repo.get!(Text, id)

  @doc """
  Creates a text.

  ## Examples

      iex> create_text(%{field: value})
      {:ok, %Text{}}

      iex> create_text(%{field: bad_value})
      {:error, %Ecto.Changeset{}}

  """

  @doc """
  Updates a text.

  ## Examples

      iex> update_text(text, %{field: new_value})
      {:ok, %Text{}}

      iex> update_text(text, %{field: bad_value})
      {:error, %Ecto.Changeset{}}

  """
  def update_text(%Text{} = text, attrs) do
    text
    |> Text.changeset(attrs)
    |> Repo.update()
  end

  @doc """
  Deletes a text.

  ## Examples

      iex> delete_text(text)
      {:ok, %Text{}}

      iex> delete_text(text)
      {:error, %Ecto.Changeset{}}

  """
  def delete_text(%Text{} = text) do
    Repo.delete(text)
  end

  @doc """
  Returns an `%Ecto.Changeset{}` for tracking text changes.

  ## Examples


      iex> change_text(text)
      %Ecto.Changeset{data: %Text{}}

  """
  def change_text(%Text{} = text, attrs \\ %{}) do
    Text.changeset(text, attrs)
  end
end
